import { AggregatedAnswerScore, Answer, CompanyAggregatedAnswersQuery, CompanyISIN, DatetimeString, QuestionId } from '../../../models'
...
export default async (req, res) => {
    ...
    const answers = loadAnswers(
        [answerDateRange(startDate, endDate)]
    )
    ...
    return res.status(200).send(
        Array.from(
            scores.values()
                .filter(score => score.count > (minCount || 0))
        )
    )
}
...
function calculateScoreKey(answer: Answer): ScoreKey | undefined {
    try {
        ...
        console.log(created, startDate, endDate)
        return [answer.company.isin, answer.question.id, startDate.toISOString(), endDate.toISOString()]
    } catch {
        console.warn(`Answer without valid creation datetime: id=${answer.id}, created=${answer.created}`)
        return
    }
}
